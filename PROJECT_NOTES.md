...
## Bug Fixes & Hardening (2026-02-20)
69. **Fill detection epsilon** — `_confirm_fill` used strict `size_matched >= original_size` comparison. Polymarket's API sometimes returns `size_matched=7.999997` for an 8-share order (floating-point representation). This caused the bot to run the full 20s timeout + cancel before falling back to balance detection. Fixed: `size_matched >= original_size - 0.001`. Side effect: also eliminates the balance API flood triggered by the timeout window.
70. **Balance fallback API flood** — when the target GTC sell had `status=MATCHED` and `size_matched=0` (an occasional Polymarket API artifact), the balance fallback in `_monitor_position` fired every 2s monitor cycle — 60+ REST calls during a single partial-fill window. Added `last_bal_check_ts` to `IntervalState` and rate-limited the fallback to once per 10 seconds.
71. **exit_reason mislabeled in below-min sell path** — `_sell_position` hardcoded `"target_fill"` as the exit reason when called with `reason="forced_exit"` but the order value was below $1. Now uses the actual `reason` parameter.
72. **`_sell_in_progress` deadlock** — the flag was set to `True` on entry to `_sell_position`, but only cleared in the `finally` block of the inner order-placement `try`. Two early `return` paths (order value < $1 minimum, shares < POLY_MIN_SHARES) exited the function without resetting the flag. This permanently locked out all subsequent sell attempts — TEMA retries, forced exit, everything — for the rest of the interval. Position expired worthless instead of recovering whatever cents were available. Fixed by adding `iv._sell_in_progress = False` before each early return. Root cause confirmed in live log (2026-02-20 03:25 UTC): 150+ "Sell already in progress" spam while shares sat at $0.05 with 5 minutes remaining.

73. **Exit watchdog (feed-quiet backup)** — `_monitor_position` is only called on price ticks. If the feed stops sending ticks in the last 30s, forced exit would never run. Added a background task that every 5s checks for an open position with `remaining <= exit_before_end` and calls `_monitor_position(iv)`. Feed and watchdog run via `asyncio.gather()`. Log: `[WATCHDOG] Forced-exit window (Xs left) — triggering monitor (price feed may be stale)`.

74. **5:00–5:15 AM ET loss — real root cause** (from Railway logs): Price feed was fine. TEMA exit fired (position Up, TEMA crossed Down). CLOB bid had collapsed (0.18 → 0.16 → 0.15), so order value (5.93 shares × floor) was below Polymarket's $1 minimum. Each call to `_sell_position` hit `[SELL SKIP] Order value < $1 min … holding to resolution` and returned **without posting an order** — but the attempt counter was incremented at the **start** of `_sell_position`. After 3 such skips the bot hit "SELL GAVE UP" and marked the position exited (`tema_exit_failed`, P&L $0). Because `iv.exited` was set, the **forced exit at 30s never ran** (monitor skips when already exited). Fix: **only count an attempt when we actually post an order**. Moved the `forced_sell_attempts`/`tema_sell_attempts` increment to just before `create_order` (after order_value and POLY_MIN_SHARES checks). Skips no longer consume the budget, so TEMA retries keep trying and the forced exit at 30s still runs (and gets its own 3 attempts if bid is still < $1).

## Next Steps
- Monitor live performance on Railway
- TEMA exit calibration — observed instances of TEMA cutting winners in choppy markets; consider raising cushion, adding confirmation delay, or disabling
